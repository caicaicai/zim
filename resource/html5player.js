...
function html5playerRun(conf){
    ...
    if(this._pureaudio){
       ...
       if(this.isdisplay == 'disable'){
		  var html='<audio id="'+this.uuid+'" preload="auto" webkit-playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></audio>';
		  if(this.autostart == true)
			 html='<audio id="'+this.uuid+'" autoplay webkit-playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></audio>';
	    }
	    else if(this.isdisplay == 'enable'){
	      var html='<audio id="'+this.uuid+'" controls preload="auto" webkit-playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></audio>';
		  if(this.autostart == true)
			 html='<audio id="'+this.uuid+'" autoplay controls webkit-playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></audio>';
	    }
    }
    else{

    if(this.isdisplay == 'disable'){
	  var html='<video id="'+this.uuid+'" preload="auto" width="'+width+'" height="'+height+'" poster="'+this.adveDeAddr+'" webkit-playsinline  playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></video>';
	  if(this.autostart == true)
		 html='<video id="'+this.uuid+'" autoplay preload="auto" width="'+width+'" height="'+height+'" poster="'+this.adveDeAddr+'" webkit-playsinline playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></video>';
    }
    else if(this.isdisplay == 'enable'){
      var html='<video id="'+this.uuid+'" controls preload="auto" width="'+width+'" height="'+height+'" poster="'+this.adveDeAddr+'" webkit-playsinline playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></video>';
	  if(this.autostart == true)
		{
		 html='<video id="'+this.uuid+'" autoplay controls preload="auto" width="'+width+'" height="'+height+'" poster="'+this.adveDeAddr+'" webkit-playsinline playsinline><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></video>';
         }

       if(isAndroid==true)
    	{
html='<video style="object-position:0px 0px" id="html5Media2"   width="'+width+'" height="'+height+'" poster="'+this.adveDeAddr+'"  x5-video-player-type="h5" x5-video-player-fullscreen="true"><source src="'+this.hlsUrl+'" type="application/x-mpegURL"/></video>';	
...
$("#mybar").on('touchstart', function(event) {
   ...
   setInterval(outline, 3000);
    ...
    function outline()
    {
        //console.log(document.getElementById(_this.uuid));
